...
|---------|---------------------|
| `init` | `--defaults`, `--providers`, `--tf-tool`, `--customer-id`, `--customer-shortname`, `--billing-account-infra`, `--customer-organization-id`, `--customer-domain`, `--iac-user`, `--default-region`, `--infra-project-name`, `--infra-bucket-name`, `--workload-root-folder-name`, `--force` (rewrite an existing estate instead of merging into it), `--interview` (ask for what is still unbound) |
...
- `--workload-root-folder-name <NAME>`: the folder directly under the organisation that holds the customer's and the teams' folders. The estate declares it (`google_folder.workload_root`) and publishes its `folders/<number>` as the core export `workload_root`; without the flag the workload root is the organisation and `workload_root` is `organizations/<id>`. A folder of that name the customer already has is imported with `satz adopt <estate> --execute --import` before the first apply.
