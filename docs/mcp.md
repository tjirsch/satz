...
| `satz_interview` | `read` / `write` | the interview: the open questions (or all, with `filter: all`), each with its pack's description and its offer. With `write`: `create` writes the estate first, `answers` writes what the human decided (for a choice, the option's param name, or `none` where the choice is not required), `accept_defaults` writes every offer — and the report comes back as it now stands. An answer whose shape contradicts the question's `shape` is refused and nothing is written. `create` writes the day-0 estate: eighteen questions, and every pack's `use` line commented out under the phase it can be adopted in, from the `pack-graph.json` in `presets_dir` — with none there, the estate is written without pack lines. Answering a pack's question `true` switches its line on as `satz_add_pack` does — uncommented, or written where the pack graph places it — so an answer and what the estate uses cannot disagree; like it, the answer is refused while a pack it needs is off, and an answer that would leave an estate satz refuses writes nothing. `notices` carries what those answers opened: a pack that names a command to run once it is on ([§6.15](language.md#615-notice--the-command-a-pack-asks-for-once-it-is-on)) — its `text`, the `run`, and the `param` to answer `true` through `answers` once the command has run. `contributes` carries what the packs those answers switched on add to other packs' list params ([language: `contributes_<param>`](language.md#contributes_param--a-packs-entries-in-another-files-list)) — each with the `pack`, the `param`, its `values`, and `applied`, false when nothing the estate uses declares that param. A call that answers nothing returns neither. [satz interview](interview.md) |
...
| `satz_check_consumer` | `read` | a project's HCL beside the estate, held to the estate's interface as `satz check-consumer` holds it: `dir` names the project's directory of `.tf` files under the server's root. `findings` lists, at the project's file and line, an attachment resource onto the estate's object that no attach point (`export … attach [ … ]`) allows, an authoritative grant (`*_iam_policy`, `*_iam_binding`) or an organisation policy on a node the estate manages, and a resource the estate declares too, by natural key; an empty list is a pass. `resources` counts the project's `resource` blocks read. Offline; writes nothing |
...
| `satz_transpile_check` | `read` | compiles in memory and reports what it *would* emit — writes nothing; `findings` carries what the compile said, each at its file and line, by `kind` and `subject` — the pack, the notice's param, the action's name, the `hcl` block's `file:line` — with `message`, the sentence, and `fix`, the command that answers it, where one command does. A finding a tier silenced is in the list too, carrying `silenced` with that tier and its reason, so an agent sees what a human's output leaves out. A refusal is an error result that carries the same list — the front end's too, as one `front-end` finding — so an error is shown at its line rather than parsed out of the sentence; its text block is the errors as the CLI lays them out. `satz transpile <estate> --check --format json` prints this object |
...
| `satz_remediation_items` | `read` | the remediation dossier's items for an estate and a Prowler export — triaged, deduplicated, joined per (control, resource) — with the `dossier_sha256` authored values must name. The worklist for the `[Authored]` columns. `checkov` names a Checkov JSON report under the root — the one `satz_scan_checkov` writes to its `out` — and its findings join the dossier's; the tool reads it and runs nothing |
| `satz_remediation_annotate` | `write` | writes authored values (`what_why`, `recommended_fix`, `owner`, `effort`, `phase`, `quick_win`, `risk_acceptance`, and the mandatory `authored_by` and `authored_at`) per item id into `<out>/authored.json`, merged with what is on file, and renders the run there with the `[Authored]` columns filled. It takes the `prowler` export and the `checkov` report the items were built from. Refused when the hash is not the current dossier's, an id is unknown, or an entry names no author |
...
`satz report-compliance --format json` writes. A call with no `framework` returns one
report per framework the estate is held to, so its result is that size multiplied by
however many it names. Where an estate outgrows the limit, name one `framework` per call,
raise `MAX_MCP_OUTPUT_TOKENS` in the client's environment, or run the command and read
the file it writes.
...
- **`satz add-project` has no tool.** The interface plane's tools are paused until it has
  settled; the command edits the estate file the way `satz_add_pack` does, and a tool
  joins then.
