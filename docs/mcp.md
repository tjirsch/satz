...
| `satz_get_presets` | `write` | the upstream library into the open estate's `presets_dir`: missing files installed, identical ones left, changed ones the estate does not use refreshed; a pack the estate uses that upstream changed is refused unless `force`. A used copy that does not parse is an error naming its line; with `force` it is overwritten and listed in `forced` with its `parse_error`. `pristine_dir` copies from a library under the root instead of downloading. `presets_dir` must be inside the root |
