...
| `presets/interface-lookups.yaml` (the data source, keys, permission and attributes per type the interface modules read back) | by hand, from the provider's data source schemas (`tofu providers schema -json`) | the provider pin moves; an export needs a type the table has no row for | the rows the showcase uses: smoke, `tofu validate` of `hcl/interfaces/`; the other rows: **nothing** |
...
| the published releases | `.github/workflows/prune-releases.yml`, after every release | a release | it keeps the five newest AND the release `satz_core::LAST_YAML_CONVERTING_RELEASE` names, read out of the source at that commit — a refusal that sends an operator to a release that was pruned is a dead end, and `--cleanup-tag` takes the tag with it |
...
- `presets/docs/*.md` (46 pages + the index) — generated by `satz doc-packs`, gated
  ...
  a page stale.
- `presets/pack-graph.json` — the pack graph, generated by `satz pack-graph` from the
  map's `offers` entries and the packs, gated by `pack-graph --check` in the smoke
  matrix and by `cargo test`. A new pack needs an `offers` entry in
  `presets/estate-map.satz` before the graph can be written; the command names what is
  missing. Regenerate in the same commit as any pack change.
...
- `SITE_DOCS` — published.
- `SITE_DOCS_EXCLUDED` — not published, each with its reason.
...
The 46 per-pack pages under `presets/docs/` are rendered and linked from the
