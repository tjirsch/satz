...
| `presets/interface-lookups.yaml` (the data source, keys, permission and attributes per type the interfaces read back) | by hand, from the provider's data source schemas (`tofu providers schema -json`) | the provider pin moves; an export needs a type the table has no row for | the rows the showcase uses: smoke, `tofu validate` of `hcl/interfaces/`; the other rows: **nothing** |
