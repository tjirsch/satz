...
| `presets/README.md` (`## Changelog`) | by hand, one row per pack version | a pack version changes | smoke: fails on a version with no row |
...
- `presets/README.md` `## Changelog` — one row per pack version, by hand. The smoke matrix fails
