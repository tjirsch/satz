...
becomes one estate. Today every top-level block is carried **verbatim** inside
`hcl trust "imported from <file>:<line>" { … }`: the estate deploys exactly as the
source did and `tofu plan` against the source's state shows no changes, but the
fold cannot compose a wrapped block and the compliance plane cannot see into it.
`terraform` and `provider` blocks are dropped with a note — the emitter writes
`providers.tf` from the estate's own `terraform` and `providers` blocks. Every block
is listed as wrapped or dropped; nothing vanishes.

What will translate into Satz resources (next step, roadmap 3.1b) and what stays
wrapped:

| HCL | Becomes |
|---|---|
| `resource` of a schema-known type, literal values | a Satz resource (labels `-`→`_`, repeated blocks → a list of objects, `*_iam_member` → a grant entry, folder/project nesting recovered from literal parents) |
| a `resource` that references another resource, uses `for_each`/`count`/`dynamic`, functions, or a provider alias | stays wrapped — and so does anything that references it |
| `module`, `locals`, `data`, `variable`, `output`, `moved`, `import` | stays wrapped |
