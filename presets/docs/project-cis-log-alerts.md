<!-- generated by `satz doc-packs` from the pack file — edit the pack or the notes region, never the rest -->
# `project_cis_log_alerts` — version 1.1

Source: `presets/monitoring/project-cis-log-alerts.satz`

## Purpose

CIS log metric filters and alert policies, per project.

Covers CIS GCP Foundations 2.5 – 2.12: eight log-based metric filters, each
with an alert policy, plus the notification channel they fire into.

It declares several resource types, so it goes at the TOP level: the target
project is chosen by each resource's own project field, not by where the `use`
sits.

---------------------------------------------------------------------------
ONE PROJECT PER USE — no parameterisation
---------------------------------------------------------------------------
The labels below (cis_2_5_project_ownership, ...) are fixed, so using this
pack twice collides on the fold. For a second project, fork it and prefix
every label and metric `name` (cis_2_5_project_ownership ->
tenantb_cis_2_5_project_ownership), or — preferably — use the central variant,
which covers all projects at once:
presets/monitoring/organization-cis-log-alerts-central.satz

---------------------------------------------------------------------------
WHY PER PROJECT — and why that is the expensive option
---------------------------------------------------------------------------
Notification channels are PROJECT resources
(google_monitoring_notification_channel -> projects.notificationChannels).
An alert policy can only reference channels living in its OWN project;
there is no organization-level channel and no cross-project reference.
So "one channel for everything" is not available: using this pack in
N projects creates N channels pointing at the same mailbox, N x 8 metrics
and N x 8 policies. Every new project needs it again, or it silently fails
CIS 2.5-2.12.

The cheaper topology: route all projects' logs through an org-level
aggregated sink into a LOGGING bucket (not GCS), then define the eight
metrics bucket-scoped in that one project. Prowler credits every child
project of such a sink as compliant
(logging_service.get_projects_covered_by_aggregated_metric) — eight metrics
and one channel for the whole organization, and future projects are covered
on creation without touching their config.
See organization-cis-log-alerts-central.satz.

Use THIS pack when a project must alert on its own (separate on-call,
delegated ownership, tenant isolation) or when the central logging bucket
does not exist yet. Both variants may coexist: Prowler passes the control as
soon as EITHER condition is met.

---------------------------------------------------------------------------
WHAT PROWLER CHECKS — and what it does not
---------------------------------------------------------------------------
The checks require (a) a metric whose filter CONTAINS the CIS filter string
and (b) any alert policy whose condition filter mentions that metric name.
Whether the policy has a recipient is NEVER checked: an alert policy with
notification_channels = [] passes CIS and notifies nobody. The channel below
exists for that reason, not because the benchmark asks for it.

Filter strings must match Prowler's expectation verbatim — they are compared
by substring. Reformatting or "improving" a filter silently breaks the check
while leaving the alert working. Source of truth:
prowler/providers/gcp/services/logging/logging_log_metric_filter_and_alert_for_*
Validated against prowler master, 2026-08-18.

---------------------------------------------------------------------------
PREREQUISITES
---------------------------------------------------------------------------
- logging.googleapis.com and monitoring.googleapis.com enabled in the
target project (add them to its project_service list).
- The recipient group must exist in Cloud Identity BEFORE apply; Google
accepts unverified email channels but they stay unconfirmed and silent.
- Admin Activity audit logs are always on; DATA_READ/DATA_WRITE are not
needed for these eight filters (they all match activity logs).

Verify after transpile:
grep -c 'google_logging_metric'          hcl/main.tf   # expect 8
grep -c 'google_monitoring_alert_policy' hcl/main.tf   # expect 8
grep -A3 'notification_channels'         hcl/main.tf   # reference, not literal

After apply, prove the pipeline works end to end — a metric that never
increments satisfies the auditor but not the attacker:
gcloud iam roles create cis_probe --project=<p> --permissions=storage.buckets.get --stage=GA
gcloud iam roles delete cis_probe --project=<p>
# expect an alert from the 2.7 policy within ~5 minutes

## Use it

```
params {
  // the pack's own defaults — copy only the lines you change
  cis_alert_project = "{infra_project_name}"
  cis_alert_email_local = "gcp-security"
  cis_alert_channel_name = "CIS Security Alerts"
  cis_alert_window = "300s"
}

use "presets/monitoring/project-cis-log-alerts.satz"
```

**Needs from outside the pack:**

- `customer_domain` — declared by `estate_core`; use one of them alongside, or declare it in the estate.
- `infra_project_name` — declared by `estate_core`; use one of them alongside, or declare it in the estate.

## Params

| param | default |
|---|---|
| `cis_alert_project` | `"{infra_project_name}"` |
| `cis_alert_email_local` | `"gcp-security"` |
| `cis_alert_channel_name` | `"CIS Security Alerts"` |
| `cis_alert_window` | `"300s"` |

## Questions

What to ask before this pack is configured, and what changing the answer costs.

| asks about | prompt | reversal | blast | recommends |
|---|---|---|---|---|
| `cis_alert_project` | The one project this pack watches — its metrics, policies and channel are created there | recreate | low | — |
| `cis_alert_email_local` | The local part of the alert recipient's address (the customer domain is added) | edit | high | — |

**cis_alert_project** — One project per use: the labels are fixed, so a second project means a fork. Changing the project later recreates all eight metrics and policies in the new one and loses the alert history of the old.


**cis_alert_email_local** — Monitoring sends from alerting-noreply@google.com, so the mailbox must exist and accept external mail; a group whose members have no mailboxes drops every alert silently. A group, never a person.


## Contributes

| resource type | labels |
|---|---|
| `google_logging_metric` | `cis_2_5_project_ownership`, `cis_2_6_audit_config`, `cis_2_7_custom_role`, `cis_2_8_firewall_rule`, `cis_2_9_network_route`, `cis_2_10_network`, `cis_2_11_bucket_permission`, `cis_2_12_sql_instance` |
| `google_monitoring_alert_policy` | `cis_2_5_project_ownership`, `cis_2_6_audit_config`, `cis_2_7_custom_role`, `cis_2_8_firewall_rule`, `cis_2_9_network_route`, `cis_2_10_network`, `cis_2_11_bucket_permission`, `cis_2_12_sql_instance` |
| `google_monitoring_notification_channel` | `cis_security_alerts` |

## Claims

_None — this pack proves no control by itself._

## History

| version | date | change |
|---|---|---|
| 1.1 | 2026-09-10 | two `question` blocks: the one project this use watches, and the alert recipient's local part. Nothing emitted changes |
| 1.0 | 2026-08-21 | per-project variant of the CIS 2.5–2.12 metrics + alerts |

The whole library's history: [the changelog](../README.md#changelog) in `presets/README.md`.

## Notes

<!-- notes:start — hand-written, kept across `satz doc-packs` -->

_No notes yet._

<!-- notes:end -->
