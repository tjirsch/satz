...
# `CIS_GCP_Foundation_4_0` — version 2.9
...
params {
  ...
  cis_block_internet_ssh_rdp = false
...
**cis_block_internet_ssh_rdp** — A firewall policy on the organisation is read before every VPC rule: each session that reaches an instance straight from the internet on port 22 or 3389 ends the moment it is attached, unless its source is in the listed ranges. Move such access to IAP first.
