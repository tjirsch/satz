...
# `scc_service_enablement` — version 1.1
...
|---|---|---|---|---|
| `scc_optional_services` | Which detectors outside the baseline does this organisation want: leave, all, none, or the ones you name? | edit | low | — |

**scc_optional_services** — Neither is in the baseline: Web Security Scanner actively crawls the customer's web applications, and Artifact Analysis is billed per image scan. `leave` keeps whatever the organisation already has — which is also how one somebody switched on in the console survives. Naming them makes the estate the source of truth: the ones named are on and the rest go off.
