...
`use_interface_notice` is the gate: while it is falsy the pack contributes nothing — no resources, no params, no claims. It is declared by `estate_map`, and the estate's own binding wins.
