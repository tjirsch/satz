...
# no --workload-root-folder-name: the organisation is the workload root, published, nothing created
grep -q '^export "workload_root" = "organizations/{customer_organization_id}"' tmp/init/satz/C0example.satz \
  && ! grep -q 'google_folder.workload_root' tmp/init/satz/C0example.satz \
  || fail "satz init without a workload root folder must export the organisation and declare no folder"
