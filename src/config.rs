...
pub struct ImportResourceConfig {
    ...
    /// API path → Terraform path for the fields whose names differ (F5c).
    /// Generated by `satz map-types` into `type-map.yaml`, merged in at load;
    /// applied by the live import before the schema filter.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub map: Option<std::collections::BTreeMap<String, String>>,
    /// The Discovery schema id to align against when the asset type's name
