...
/// The `use` clause of `l` as it reads gated on `gate` — what a finding tells the operator
/// to write on a line that lacks its `when`.
fn gated_line(l: &UseLine, gate: &str) -> String {
    match &l.alias {
        Some(a) => format!("use \"{}\" as {} when {}", l.written, a, gate),
        None => format!("use \"{}\" when {}", l.written, gate),
    }
}
...
impl<'a> View<'a> {
    ...
    fn line_findings(&self, label: &str, level: &str) -> Vec<(String, Finding)> {
        ...
        for n in &self.graph.nodes {
            ...
            if let ("ungated", Some(l), true) = (state, line, self.gate_exists(n)) {
                let g = n.gate.as_deref().unwrap_or_default();
                ungated.push((
                    n.path.clone(),
                    format!(
                        "`{}` is used without `when {}`, so a no to `{}` does not switch it off — write `{}` on that line and bind `{} = true` in this estate's own `params {{ }}`: the line deploys the pack, and the binding keeps it deploying",
                        n.path, g, g, gated_line(l, g), g
                    ),
                    None,
                    at(line),
                ));
    ...
    for n in &off {
        ...
        match &l.when {
            None => refusals.push(format!(
                "`{}` is used without `when {}` at line {}, so binding `{}` false does not switch it off — write `{}` there first",
                n.path, g, l.index + 1, g, gated_line(l, g)
            )),
...
mod tests {
    ...
    fn an_ungated_line_is_found_only_where_its_gate_exists() {
        ...
        // the finding states the edit: no command writes the `when`
        let edit = "write `use \"presets/organization-budget.satz\" when use_budget` on that line and bind `use_budget = true`";
        assert!(messages(&src).iter().any(|m| m.contains(edit) && !m.contains("merge-presets")), "{:?}", messages(&src));
